...
describe('main handler smoke tests', () => {
  ...
  it('should send out a "success" build message', async () => {
    ...
    expect(stdout).toEqual(
      expect.stringMatching(
        /Outbound message: GitHub build \*\*Testing Production Deployment\*\* \(tag v0.0.1\) for repository `marvinpinto\/private-actions-tester` completed successfully :tada: - https:\/\/git.io\/fake1234/,
      ),
    );
